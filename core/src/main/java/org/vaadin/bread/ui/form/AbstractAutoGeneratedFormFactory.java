 ...
 */
public abstract class AbstractAutoGeneratedFormFactory<T, E extends Enum<E>> extends AbstractFormFactory<T> implements FormFactory<T> {

    protected Map<Operation, String> buttonCaptions = new HashMap<>();
    protected Map<Operation, Resource> buttonIcons = new HashMap<>();
    protected Map<Operation, Set<String>> buttonStyleNames = new HashMap<>();
...
//    protected String cancelButtonCaption = "Cancel";
    ...
    public AbstractAutoGeneratedFormFactory(Class<T> domainType, Operation[] operations) {
    	super(operations);
        this.domainType = domainType;

        for (Operation operation : operations) {
        	setButtonCaption(operation, operation.getOperationName());
    ...
    public void setButtonCaption(Operation operation, String caption) {
        buttonCaptions.put(operation, caption);
    }

    public void setButtonIcon(Operation operation, Resource icon) {
        buttonIcons.put(operation, icon);
    }

    public void addButtonStyleName(Operation operation, String styleName) {
    ...
    protected List<HasValue> buildFields(Operation operation, T domainObject, boolean readOnly) {
        binder = buildBinder(operation, domainObject);
        ArrayList<HasValue> fields = new ArrayList<>();
        FormConfiguration configuration = getConfiguration(operation);
    ...
    protected Binder<T> buildBinder(Operation operation, T domainObject) {
        Binder<T> binder;

        if (getConfiguration(operation).isUseBeanValidation()) {
            if (getConfiguration(operation).getJpaTypeForJpaValidation()!=null) {
            	binder = new JpaValidationBinder(getConfiguration(operation).getJpaTypeForJpaValidation(), domainObject.getClass());
            } else {
    ...
    protected Button buildOperationButton(Operation operation, T domainObject, Button.ClickListener clickListener) {
        if (clickListener == null || operation==null) {
            return null;
        }

        Button button = new Button(buttonCaptions.get(operation), buttonIcons.get(operation));
        Set<String> set = buttonStyleNames.get(operation);
        if (set!=null) {
        ...
        button.addClickListener(event -> {
            if (domainObject==null || binder.writeBeanIfValid(domainObject)) {
                try {
                    ...
                    showError(operation, domainObject, e);
                }
            } else {
    ...
    @Override
    public void showError(Operation operation, T domainObject, Exception e) {
        if (errorListener != null) {
            errorListener.onError(operation, domainObject, e);
        } else {
            if (OperationException.class.isAssignableFrom(e.getClass())) {
                Notification.show(e.getMessage(), Notification.Type.ERROR_MESSAGE);
            } else {
                Notification.show(e.getMessage(), Notification.Type.ERROR_MESSAGE);
                throw new RuntimeException("Error executing " + operation + " operation on " +domainObject, e);
            }
        }
    }
